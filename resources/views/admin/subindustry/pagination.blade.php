<table class="table text-center" id="data-table">
    <thead>
        ...
        @if(auth('admin')->user()->can('sub-industry-edit') || auth('admin')->user()->can('sub-industry-delete'))
        <th class="btns-col">Action</th>    
        @endif    
    </thead>
    <tbody>
        @forelse ($subindustryData as $industry)
            <tr id="item_{{ $industry->id }}">
                <td scope="row" class="w-8 name-col">{{ $industry->title }}</td>
                <td scope="row" class="w-10">
                    @if(auth('admin')->user()->can('sub-industry-edit'))
...
<div id="sub-industry_nav">
    <div class="d-flex align-items-center justify-content-between">
        <div>
            Total no. of records = <span class="text-theme-color">{!! $subindustryData_count !!}</span>
        </div>
        <div>{{ $subindustryData->links() }}</div>
    </div>
</div>