<table class="table text-center" id="data-table">
    <thead>
        ...
        @if(auth('admin')->user()->can('insight-edit') || auth('admin')->user()->can('insight-delete'))
        <th class="btns-col">Action</th>  
        @endif      
    </thead>
    <tbody>
        @forelse ($insights as $insight)
            <tr id="item_{{ $insight->id }}">
                <td scope="row" class="w-8 name-col">{{ $insight->name }}</td>
                <td scope="row" class="w-10">
                    @if(auth('admin')->user()->can('insight-edit'))
                    <a href="javascript:void(0);" data-id="{{$insight->id}}" class="btn action-btn updateStatus" role="button" aria-pressed="true">
                    @endif
                ...
                <td>
                    @if(auth('admin')->user()->can('insight-edit'))
                    <a id="edit_row" href="{{ url('admin/insight/edit/' . $insight->id) }}"
                        class="btn action-btn" role="button" aria-pressed="true" title="Edit">                        
                        <i class="fa fa-pen green" aria-hidden="true"></i>
                    </a>
                    @endif
                    @if(auth('admin')->user()->can('insight-delete'))
                    <a href="javascript:void(0);" data-id="{{ $insight->id }}" class="btn action-btn deleteData" title="Delete" role="button" aria-pressed="true">
...
<div id="insight_nav">
    <div class="d-flex align-items-center justify-content-between">
        <div>
            Total no. of records = <span class="text-theme-color">{!! $insights_count !!}</span>
        </div>
